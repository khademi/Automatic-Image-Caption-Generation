...
hyperparameters = {'batch_size': 100, 'save_freq': 100, 'model': 'grid_icg',
                   'word_emb_size': 512, 'feat_dim': 2048, 'grid_size': 14,
                   'dropout_keep_rate': 0.5, 'learning_rate': 0.00001,
                   'grid_feat_dim': 1024, 'grid_emb_size': 512,
                   'vocab_size': len(coco_dict)}
...
features = tf.parse_single_example(
    ...
    features={
        ...
        'visual_concept': tf.FixedLenFeature([VISUAL_CONCEPT_SIZE], tf.float32),
        'grid_feat': tf.FixedLenFeature([grid_feat_dim * grid_size * grid_size],
                                        tf.float32),
...
saver = tf.train.Saver(max_to_keep=1, keep_checkpoint_every_n_hours=100)

# Run the training session and save the results 
try:
    step = 0
    total_cost = 0.0
